...
const EventItem = props=>{
        ...
        const fetchCommentsHandler=async (id)=>{
            ...
            try{
                ...
                if(!loadedComments){
                    ...
                    const responseData=await sendRequest(`${process.env.REACT_APP_BACKEND_URL}/events/comment/${id}`);
    ...
    return (
        ...
        {loadedComments && !openComments && <button onClick={openCommentsHandler} className="comment_button">{loadedComments.length} {loadedComments.length===1?'comment':'comments'}</button>}

        {loadedComments && openComments && <Comments comments={loadedComments} setOpenComments={openCommentsHandler} deleteHandler={commentDeleteHandler}/>}
